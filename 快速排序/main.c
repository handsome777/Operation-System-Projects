...
void main()
{
	...
	__time_t t_begin = ClockGetTime();//begin time
	//init mutex and cond
	int error = 0;
...
void* worktime(void* param)
{
	...
	int left = q_n->begin;
	int right = q_n->end;
	if(right - left <= SORT_MIN_NUM)//if subsequence length <= 1000
	{
		qsort(&data_double[left],right - left + 1,sizeof(double),comp);//use qsort
